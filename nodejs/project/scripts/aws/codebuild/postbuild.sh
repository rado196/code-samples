...
aws deploy push \
  ...
  --s3-location s3://$CODE_BUILD_S3_BUCKET/$CODE_BUILD_S3_FOLDER/api-source.zip
...
aws deploy create-deployment \
  --application-name "$CODE_BUILD_DEPLOY_APP_NAME" \
  --s3-location bucket=$CODE_BUILD_S3_BUCKET,key=$CODE_BUILD_S3_FOLDER/api-source.zip,bundleType=zip \
